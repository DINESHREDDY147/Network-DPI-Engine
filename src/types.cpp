...
// Helper: match keyword only at domain boundaries (preceded by '.' or start,
// followed by '.' or end). Prevents "t.co" matching inside "microsoft.com".
static bool matchDomainKeyword(const std::string& domain, const char* keyword) {
    size_t klen = std::strlen(keyword);
    size_t pos = 0;
    while ((pos = domain.find(keyword, pos)) != std::string::npos) {
        bool left_ok  = (pos == 0 || domain[pos - 1] == '.');
        bool right_ok = (pos + klen >= domain.size() || domain[pos + klen] == '.');
...
AppType sniToAppType(const std::string& sni) {
    ...
    // Twitter/X — use boundary matching for short patterns to avoid
    // "microsoft.com" matching "t.co" or "netflix.com" matching "x.com"
    if (lower_sni.find("twitter") != std::string::npos ||
        lower_sni.find("twimg") != std::string::npos ||
        matchDomainKeyword(lower_sni, "x.com") ||
        matchDomainKeyword(lower_sni, "t.co")) {
