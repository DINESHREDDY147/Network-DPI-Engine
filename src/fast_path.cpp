...
void FastPathProcessor::run() {
  while (running_) {
    ...
    // Recompute payload pointer (may be invalidated by queue move)
    PacketJob &job = *job_opt;
...
PacketAction FastPathProcessor::checkRules(const PacketJob &job,
  ...
  if (block_reason) {
    // Log the block — build complete string then write atomically
    ...
    // Single write call to minimize interleaving
